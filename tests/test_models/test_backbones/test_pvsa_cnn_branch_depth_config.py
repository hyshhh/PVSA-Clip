...
def test_vtformer_exposes_transformer_branch_depth_config():
    root = Path(__file__).resolve().parents[3]
    source = (
        root / 'mmseg' / 'models' / 'backbones' / 'bi_topp_vote.py'
    ).read_text(encoding='utf-8')
    ...
    assert 'self.transformer_branch_depth[0]' in source
    assert 'self.transformer_branch_depth[i + 1]' in source
    assert 'transformer_branch_depth=[0, 0, 0, 0]' in config
